object YCadItem: TYCadItem
  Left = 679
  Top = 239
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'YCadItem'
  ClientHeight = 261
  ClientWidth = 612
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = PopupMenu1
  Position = poMainFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  DesignSize = (
    612
    261)
  PixelsPerInch = 96
  TextHeight = 13
  object BitOk: TBitBtn
    Left = 532
    Top = 208
    Width = 75
    Height = 25
    Cursor = crHandPoint
    Anchors = [akTop, akRight]
    Caption = '&Ok'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    OnClick = BitOkClick
    OnMouseMove = BitOkMouseMove
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object BitCancelar: TBitBtn
    Left = 451
    Top = 208
    Width = 76
    Height = 25
    Cursor = crHandPoint
    Anchors = [akTop, akRight]
    Caption = '&Cancelar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    OnClick = BitCancelarClick
    OnMouseMove = BitCancelarMouseMove
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 242
    Width = 612
    Height = 19
    Panels = <
      item
        Width = 200
      end
      item
        Width = 50
      end>
  end
  object pnlLog: TPanel
    Left = 0
    Top = 236
    Width = 612
    Height = 6
    Align = alBottom
    BevelOuter = bvNone
    Ctl3D = False
    Enabled = False
    ParentCtl3D = False
    TabOrder = 3
    object lblInclusao: TLabel
      Left = 8
      Top = 7
      Width = 43
      Height = 13
      Caption = 'Inclus'#227'o:'
    end
    object lblAlteracao: TLabel
      Left = 8
      Top = 31
      Width = 45
      Height = 13
      Caption = 'Altera'#231#227'o'
    end
    object eDataInc: TDBEdit
      Left = 61
      Top = 4
      Width = 113
      Height = 19
      BevelOuter = bvNone
      Color = clBtnFace
      Ctl3D = False
      DataSource = dsMultiplaInstancia
      ParentCtl3D = False
      TabOrder = 0
    end
    object eDataAlt: TDBEdit
      Left = 61
      Top = 25
      Width = 113
      Height = 19
      BevelOuter = bvNone
      Color = clBtnFace
      Ctl3D = False
      DataSource = dsMultiplaInstancia
      ParentCtl3D = False
      TabOrder = 1
    end
    object eUserInc: TDBEdit
      Left = 177
      Top = 4
      Width = 413
      Height = 19
      BevelOuter = bvNone
      Color = clBtnFace
      Ctl3D = False
      DataSource = dsMultiplaInstancia
      ParentCtl3D = False
      TabOrder = 2
    end
    object eUserAlt: TDBEdit
      Left = 177
      Top = 25
      Width = 413
      Height = 19
      BevelOuter = bvNone
      Color = clBtnFace
      Ctl3D = False
      DataSource = dsMultiplaInstancia
      ParentCtl3D = False
      TabOrder = 3
    end
  end
  object dsMultiplaInstancia: TDataSource
    Left = 72
    Top = 88
  end
  object gravaLog: TGravaLog
    Ativada = False
    UserCreate = 'USER_CREATE'
    DataCreate = 'DT_CREATE'
    UserAlter = 'USER_ALTER'
    DataAlter = 'DT_ALTER'
    UsuarioLogado = 'AUTO'
    DataAlteracao = 'AUTO'
    DataInclusao = 'AUTO'
    Left = 44
    Top = 88
  end
  object ControleTela: TControle
    CamposMaiusculos = False
    AutoIncremento = False
    AutoAjusteLengthCampoTela = False
    PermiteRelatorioPadrao = True
    DesabilitaCampoChave = True
    CorCampoChaveDesabilitado = clBtnFace
    CorCampoChaveHabilitado = clWindow
    Left = 16
    Top = 88
  end
  object PopupMenu1: TPopupMenu
    Left = 104
    Top = 88
    object Solicitao1: TMenuItem
      Caption = 'Solicita'#231#227'o'
      OnClick = Solicitao1Click
    end
  end
end
